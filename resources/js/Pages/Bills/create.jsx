...
export default function Create({ patients, appointments }) {
    ...
    return (
        <AuthenticatedLayout
            header={<h2 className="text-lg font-semibold text-gray-800">Yangi to'lov qo'shish</h2>}
        >
            ...
            <div className="max-w-2xl mx-auto">
                <form onSubmit={submit} className="bg-white rounded-xl shadow-sm border border-gray-200 p-6 space-y-4">
                    ...
                    <div>
                        <label className="block text-sm text-gray-600 mb-1">Tavsif (ixtiyoriy)</label>
                        <input
                            type="text"
                            value={data.description}
                            onChange={e => setData('description', e.target.value)}
                            className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-gray-900"
                            placeholder="Masalan: Konsultatsiya, Tahlil..."
                        />
                    </div>
